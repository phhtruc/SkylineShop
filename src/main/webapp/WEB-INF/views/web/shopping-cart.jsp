...
<body>
					...
					<div class="breadcrumb__text">
						...
						<div class="breadcrumb__links">
							<a href="<c:url value='/trang-chu'/>">Trang Chủ</a> <a
								href="<c:url value='/shop'/>">Cửa Hàng</a> <span>Giỏ Hàng</span>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="row">
									...
									<tr>
										<td class="product__cart__item">
											<div class="product__cart__item__pic">
												<a
													href="<c:url value='/shop-details/${c.value.product.product_name}'/>"><img
													style="width: 90px"
													src="<c:url value='/template/web/images/${c.value.product.image }'/>"
													alt="" /></a>
											</div>
											<div class="product__cart__item__text">
												<a
													href="<c:url value='/shop-details/${c.value.product.product_name}'/>"><h6>${c.value.product.product_name}</h6></a>
												<h5>
													<fmt:formatNumber type="number" groupingUsed="true"
														value="${c.value.product.price}" />
												</h5>
											</div>
										</td>
										<td class="quantity__item">
											<div class="quantity">
												<div class="pro-qty-2">
													<span class="fa fa-minus dec qtybtn edit-cart"
														data-product-id="${c.value.product.id_product}"></span> <input
														type="text" value="${c.value.quantity }"
														id="quantity-${c.value.product.id_product}" /> <span
														class="fa fa-plus inc qtybtn edit-cart"
														data-product-id="${c.value.product.id_product}"></span>
												</div>
											</div>
										</td>
										<td class="cart__price"
											id="cart-price-${c.value.product.id_product}"><fmt:formatNumber
												type="number" groupingUsed="true"
												value="${c.value.totalPrice }" /> VND</td>
										<td class="cart__close"><a
											href="<c:url value='/delete-cart/${c.value.product.id_product }'/>"><i
					...
					<div class="row">
						...
						<div class="col-lg-6 col-md-6 col-sm-6">
							<div class="continue__btn update__btn">
								<a href="<c:url value='/shopping-cart'/>"><i
									class="fa fa-spinner"></i> Cập nhật giỏ hàng</a>
							</div>
						</div>
					</div>
				</div>
				<div class="col-lg-4">
					<div class="cart__discount">
						...
						<span id="discountMessage" style="color: #888888"></span>
						<form id="voucher">
							<input type="text" placeholder="Mã giảm giá" name="id-voucher" value="${voucherCode}"/>
					...
					<div class="cart__total">
						<h6>Tổng giỏ hàng</h6>
						<ul>
							<li>Tổng tiền<span class="cart-price"><fmt:formatNumber
						...
						<a href="<c:url value='/checkout'/>" id="checkoutLink" class="primary-btn">Tiếp tục
							thanh toán</a>
					</div>
				</div>
			</div>
	...
	<script>
	    ...
	    $('#submit').click(function (e) {
	        ...
	        var totalPriceString = document.querySelector('.cart-price').textContent.trim();
	     	var totalPriceValue = parseFloat(totalPriceString.replace(/[^\d.]/g, ''));
	     	
	        var voucherId = $('input[name="id-voucher"]').val();
	        ...
	        $.ajax({
	            ...
	            success: function (jsonResponse) {
	                ...
	                var st = jsonResponse.status;
	                
	                $('.cart-price-discountAmount').text(new Intl.NumberFormat('vi-VN').format(discountAmount) + ' VND');
	                
	                $('.price').text(new Intl.NumberFormat('vi-VN').format(totalPriceValue - discountAmount) + ' VND');
		...
		$(document).ready(function() {
		  $(".edit-cart").on("click", function() {
		    ...
		    var priceString = $(".cart-price-discountAmount").text();
		    var priceWithoutComma = parseInt(priceString.replace(/\D/g, ''), 10);
		 	var PriceA = parseInt(priceWithoutComma);
		    ...
		    $.ajax({
		      ...
		      success: function(response) {
		          ...
		          $('.price').text(new Intl.NumberFormat('vi-VN').format(cartPrice - PriceA) + ' VND');

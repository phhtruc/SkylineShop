				...
				<div class="page-content">
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<div class="row">
								<div class="col-xs-12">
									<h3 class="header smaller lighter blue">Quản lý đơn hàng</h3>
...
<script>
	$(document).ready(function(){
		$.ajax({
			url : 'http://localhost:8888/SkylineShop/api/admin/orders',
			type : 'GET',
			dataType : 'json',
		...
		function displayProducts(response) {
			...
			for (var i = 0; i < response.length; i++) {
				...
				var productNameCell = $('<td>').html('<a href="#">'+'#'+ order.id_order+ '</a>');
				row.append(productNameCell);
				// Thêm giá (ví dụ: <td>$60</td>)
				var priceCell = $('<td>').text('' + order.date);
				row.append(priceCell);
				//Thêm mô tả <td class="hidden-480">4,400</td>
				var desc = $('<td class="hidden-480">').html('<a href="#">'+ order.fullName+ '</a>');
				row.append(desc);
				// Thêm brand <td>Mar 11</td>
				var brand = $('<td>').text('' + order.payment_status);
				row.append(brand);
				//Thêm danh mục <td class="hidden-480"><span class="label label-sm label-warning">Expiring</span></td>
				var cate = $('<td class="hidden-480">').text('' + order.delivery);
				row.append(cate);
				//total
				var total = $('<td class="hidden-480">').text('' + order.total_amount);
				row.append(total);
